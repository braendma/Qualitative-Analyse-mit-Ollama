...
def extended_setup(root):
    ...
    from person_comparison_core import normalize_comparison
    from runtime_support import fingerprint
    comparison = normalize_comparison({'gemeinsame_muster': [], 'typen': [], 'zentrale_unterschiede': [],
        'nicht_zugeordnete_personen': [], 'gesamtvergleich': ''}, sorted(people))
    atomic_json(root/'person_comparison_v1.json', {**comparison, 'created_at': 'synthetic-comparison',
        'source_persons': sorted(people), 'source_person_analysis_created_at': persons['created_at'],
        'input_reduction': {'used': False, 'source_sha256': fingerprint(people)}})
...
class ExtendedThematicProcessTests(unittest.TestCase):
    def test_eight_cli_modules_html_diagnostics_person_scopes_sources_and_resume(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            path.write_text(yaml.safe_dump(cfg,allow_unicode=True),encoding='utf-8')
            env={**clean_environment(),'PYTHONUTF8':'1','MPLBACKEND':'Agg','MOCK_AMBIGUITY_PAIRS':'1','MOCK_CONTRAST_PATTERNS':'1'}
