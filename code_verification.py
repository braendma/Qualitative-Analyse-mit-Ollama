...
def main(argv=None):
    ...
    parser.add_argument("--out-md", default="code_verification_v1.md")
    parser.add_argument("--out-json", default="code_verification_v1.json")
    parser.add_argument("--log-file", default="code_verification.log")
    parser.add_argument("--mock-responses-json", default=None, help=argparse.SUPPRESS)
    args = parser.parse_args(argv)
    configure_logging(args.log_file)
    LOGGER.info("Code-Verifikation gestartet")
    ...
    markdown, output = verify_segments(
        ...
        idmap_reference=args.idmap_json, raw_log_path=raw_log_path,
        **({"llm": llm} if llm else {})
    )
    Path(args.out_md).write_text(markdown, encoding="utf-8")
    Path(args.out_json).write_text(json.dumps(output, ensure_ascii=False, indent=2), encoding="utf-8")
    LOGGER.info(
        "Code-Verifikation abgeschlossen: %s Segmente, %s Codepfade",
        len(segments),
        len(codebook),
    )
...
if __name__ == "__main__":
    try:
        ...
        LOGGER.exception("Code-Verifikation fehlgeschlagen")
        raise
