# Prüfung des Entwicklungskandidaten für 0.5.1 · 21. September 2026

1007 Python-Tests ausgeführt: 1005 bestanden, zwei vorgesehene Skips. Im ersten Sandboxlauf scheiterten nur die beiden Windows-DPAPI-Prüfungen; dieselben Tests bestanden unverändert im regulären Benutzerkontext. 96 JavaScript-Tests bestanden. Echte Offline-Chrome-Prüfungen belegen Berichtseditor, gesperrte Zitate/Tabellen, elementgenaue Meta-SWOT-Navigation, dokumentierte Folgeprüfungen, Download/Wiederöffnen und eingeschränkten App-iframe. Eine Source-Oberfläche wurde mit HTTP 200, authentifiziertem Zustand, regulärem bestätigtem Abschluss und anschließender Datenbereinigung geprüft. Der ältere Source-Smoke scheiterte zunächst bei erzwungener Prozessbeendigung an einer kurz gesperrten Logdatei; regulärer App-Abschluss wird nun genutzt.

Der separate reguläre Ollama-Cloud-Lauf mit Gemma4:31b (21. September, 08:43–08:49) bestand 20/20 Module auf sieben künstlichen Codierzeilen, sechs Passagen, drei erfundenen Personen und zwei Kategorien. Beide Perspektiven wurden in allen zehn geeigneten Modulen erzeugt. Stabilität: zwei Wiederholungen. Sensitivität: Basis und kleinere Antwortgrenze mit je zwei Wiederholungen. Insgesamt 184 physische Chatantworten, keine fehlgeschlagenen oder offenen Requests; 95 registrierte Outputs und sieben Laufmanifeste hashgeprüft. Alle sechs diagnostischen Kindläufe endeten mit bestätigter Bereinigung. Der echte Gesamtbericht enthält 20 Bereiche und 172 eindeutig bearbeitbare Deutungsfelder; Bearbeitung, Navigation, geschützte Zitate, Export/Wiederöffnen und App-iframe wurden nochmals im Offlinebrowser geprüft.

Das ist ein technischer Funktionsnachweis auf einem kleinen synthetischen Material, kein empirischer Modellgütenachweis. Gezielte Originalprüfung fand Überdehnungen: fehlende Reaktion wurde als Systemabsturz bezeichnet; eine zugleich hilfreiche Anleitung und ein Startproblem wurden als Nachweis unwirksamer Problemlösung gedeutet. Diese Aussagen bleiben menschlich zu prüfen und wurden nicht durch nachträgliche Modell-Perfektionsläufe ersetzt. Cloudgewichte und tatsächliche serverseitige Parameterdurchsetzung sind nicht unabhängig verifizierbar.

Zusätzliche Kontextprüfung: 184 gespeicherte echte Requests und 15 künstliche ASCII-/Umlaut-/Emoji-Fälle wurden an der unveränderten konservativen Budgetgrenze geprüft. 398 Prüfungen belegen: Eine Einheit unter dem errechneten Bedarf wird vor dem Transport abgewiesen, die genaue Grenze erlaubt denselben Request. Diese Replays verwenden kontrollierte Antworten und sind keine zusätzlichen Modelltests. Der anschließende Anzeige-/Fehlerhilfepatch bestand 31 gezielte Python-Testausführungen und 97 JavaScript-Tests; Grenzrechnung, Prompts und Transport blieben unverändert.

Sechs zusätzliche echte synthetische Gemma4:31b-Extraktions-/Klassifikationsfälle bestanden den Vergleich sämtlicher Antwortfelder: wachsende Eingaben bis 963 Quellen, leere Eingabe, widersprüchliche Aussagen derselben Person mit ähnlichen Kennungen sowie zitierte Anweisungen und Unicode. Der größte Fall meldete 56.976 Eingabetokens beim Anbieter; die konservative Programmrechnung einschließlich 2.048 Antwortreserve betrug 193.154. Das sind unterschiedliche Größen, keine zwei exakten Tokenmessungen. Die sechs Einzelfälle belegen weder statistische Zuverlässigkeit noch alle Module auf großen Datensätzen. Das maximale Cloudfenster wurde nicht bestimmt oder verkleinert; aus dem kleinen Gesamtlauf werden keine allgemeinen Mindestbudgets je Modul abgeleitet.

Der getrennte vollständige lokale Granite4.2:30b-Nachtest ist weiterhin offen. Finale 0.5.1-Versionierung und reine CI-/Teständerungen verändern keine Modellimplementierung oder Prompts; die Quellenübergabe wird getrennt gebunden. Vor Veröffentlichung müssen die drei tatsächlichen Pakete (Windows-Python, macOS-Python, Windows-Standalone) aus demselben finalen Commit und alle relevanten CI-Läufe dieses exakten Commits erfolgreich geprüft sein. Diese noch ausstehende Paketabnahme wird durch den beschriebenen Entwicklungstest nicht vorweggenommen.
