...
1. **Windows-Paket:** Das vollständige Windows-Paket in einen neuen Ordner entpacken und `QualitativeAnalyse.exe` öffnen. EXE und `_internal` zusammenlassen. Python ist enthalten; keine eigene Python-Installation oder CMD-Einrichtung nötig. [Windows-Kurzanleitung](WINDOWS_STANDALONE.txt).
2. Das Startfenster geöffnet lassen. Zum Beenden die Funktion **Programm beenden** in der Oberfläche verwenden; ein geschlossener Browsertab beendet das Programm nicht.
3. Für lokale KI-Analysen separat Ollama und ein lokales Modell bereitstellen. Das Paket installiert beides nicht automatisch. Der Speicherbedarf hängt vom Modell und Kontextfenster ab.
4. Unter **Analyse → Systemprüfung ohne Modellaufruf** die Einrichtung prüfen. **Kurzen Modelltest vorbereiten** ist eine getrennte, optionale Aktion mit einer künstlichen Modellanfrage.

**Source-Version:** Das gesamte Repository einschließlich Unterordnern verwenden. Python ab 3.10 bereitstellen, unter Windows einmal `Einrichtung.cmd` und danach `Start_Oberflaeche.cmd` öffnen. Die Einrichtung benötigt Internet für Python-Pakete, startet aber keine Analyse. Alternativ `python -X utf8 src/local_app.py` aus einer passenden Python-Umgebung starten. [Anleitung für Entwickler zum Windows-Build](https://github.com/braendma/Qualitative-Analyse-mit-Ollama/blob/main/packaging/README.md).

**macOS:** Eine native Mac-Distribution ist pausiert. Die bisherigen Source-Starter bleiben verfügbar; [Voraussetzungen und Testgrenzen](https://github.com/braendma/Qualitative-Analyse-mit-Ollama/blob/main/start/macos/README.md) beachten.
...
**Systemprüfung und Reparaturhinweise:** Die Source-Version verwendet deine eingerichtete Python-Umgebung. Fehlt dort eine Programmabhängigkeit, nennt die Prüfung unter Windows `Einrichtung.cmd`, unter macOS `start/macos/Einrichtung.command`. Für ein gebündeltes Windows-Programmpaket ist dagegen Python im Paket enthalten: Fehlende Abhängigkeiten erfordern, das vollständige Paket erneut herunterzuladen, in einen neuen Ordner zu entpacken und `QualitativeAnalyse.exe` daraus zu starten. Eine eigene Python-Installation repariert dessen enthaltene Laufzeit nicht. Die Systemprüfung installiert nichts und startet kein Modell. Ein nicht erreichbarer Ollama-Server betrifft lokale Modellanalysen; Oberfläche, freigegebene Cloud-Anbieter und reine Diagnosen ohne Modellbedarf bleiben nutzbar.

### Ollama-Status ohne Modellstart
...
Du findest **Programm beenden** in der Seitenleiste neben Handbuch und Telegram-Updates. Das Schließen eines Browsertabs lässt das Programm und laufende Analysen weiterlaufen. Öffne deshalb diese Funktion, wenn du auch das Programm beenden möchtest.
...
**Bei zu großem Kontext:** Originaltexte, Themenregister und vorhandene Gegenpositionen werden nicht still gekürzt. Bei einem großen Vergleichsregister kann die Anwendung wiederholte Feldnamen einmal als Tabellenspalten übertragen. Falls nötig, werden zusätzlich identische Feldwerte einmal mit ihrem Feldpfad übertragen; sie gelten ausdrücklich für jede Themenzeile. Beispielsweise muss dieselbe Einschlussregel dadurch nur einmal übermittelt werden. Alle Themen, vollständigen Definitionen, Bezugsgrößen, Zahlen und unbestimmten Werte bleiben erhalten. Diese verlustfreie Darstellung verändert weder Zählung noch Vergleichsraum. Reicht auch sie nicht aus, bleibt die Kontextprüfung wirksam. Ein einzelner zu langer Text oder ein großes Vergleichsregister kann die Prüfung deshalb stoppen. Kontextfenster und Antwortlimit anhand der Meldung prüfen; mehr Kontext muss zum Modell und verfügbaren Speicher passen. Alternativ die Eingabe methodisch begründet überarbeiten oder die zusätzliche Häufigkeitsperspektive abwählen. Keine Belege nur zur Erzielung einer gewünschten Zahl entfernen. Geänderte Daten, Modi oder Modelleinstellungen brauchen einen neuen Lauf. Bei einem rein technischen Ausfall mit unveränderter Grundlage kann das normale Fortsetzen geprüfte Teilblöcke wiederverwenden.
