...
Dieses Handbuch begleitet dich vom ersten Start bis zum erneuten Analyselauf mit geprüften Codierungen. Alle Personen, Texte und Beurteilungen in den Beispielen sind erfunden. Stand: Releasekandidat 0.5.0-beta.1, noch nicht veröffentlicht; die vollständige Windows-Installationsabnahme läuft.
...
1. **Windows-Paket:** Den vollständigen Releasekandidaten in einen neuen Ordner entpacken und `QualitativeAnalyse.exe` öffnen. EXE und `_internal` zusammenlassen. Python ist enthalten; keine eigene Python-Installation oder CMD-Einrichtung nötig. Der Kandidat ist noch nicht veröffentlicht. [Windows-Kurzanleitung](WINDOWS_STANDALONE.txt).
