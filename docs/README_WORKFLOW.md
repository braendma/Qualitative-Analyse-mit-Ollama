...
## Ablage im Entwicklungsstand P01c
...
## Kontrolliertes Beenden der Oberfläche (Entwicklungsstand P02b)

Das Schließen des Browsertabs lässt die lokale Anwendung weiterlaufen. **Programm beenden** in der Seitenleiste bietet bei Leerlauf **Programm jetzt beenden**, bei aktiver Analyse **Nach aktuellem Modul pausieren und beenden**. Für sofortiges Unterbrechen ist unter **Sofort abbrechen** die ausdrückliche Bestätigung mit **Jetzt abbrechen und beenden** erforderlich. Die Aktion ist an den tatsächlich aktiven Lauf und seinen aktuellen Startversuch gebunden; eine veraltete Auswahl darf keinen anderen Lauf unterbrechen.

Eine angeforderte Pause wartet auf die vorgesehene Modulgrenze und die anschließende Bereinigung eigener Prozesse. Bereits gespeicherte Ergebnisse bleiben erhalten. Ein späteres **Diesen Lauf fortsetzen** prüft wie bisher die ursprüngliche Konfiguration, Eingaben und Ergebnisse. **Dialog schließen** nach Annahme einer Anforderung nimmt sie nicht zurück. Neue Starts sind während des Beendens gesperrt. Bei fehlender Bereinigungsbestätigung bleibt die Anwendung mit Fehlerhinweis geöffnet; **Status erneut prüfen**, Startfenster und Laufprotokoll helfen bei der Klärung. „Bereinigung bestätigt. Das Programm wird geschlossen.“ bestätigt die Bereinigung eigener Analyseprozesse vor dem anschließenden Schließen. Ein Verbindungsabbruch allein ist kein Erfolgsnachweis.

Beim Source-Start der Oberfläche fordert Strg+C mit aktivem Lauf ebenfalls Pause und anschließendes Beenden an; auch wiederholtes Strg+C erzwingt keinen Abbruch. Ohne aktiven Lauf schließt die Anwendung geordnet. Die bestätigte UI-Aktion bleibt der Weg zum Sofortabbruch. Diese Funktion verwendet die vorhandene Prozessaufsicht für eigene Prozesse; sie beendet keine fremden Anwendungen. Noch keine Zusage getesteter neuer Windows-/macOS-Pakete. [Bedienfolge im Handbuch](HANDBUCH.md#programm-kontrolliert-beenden).
