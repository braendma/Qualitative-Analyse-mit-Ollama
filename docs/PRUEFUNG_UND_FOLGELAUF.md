...
Im Entwicklungsstand P01c liegen Prüfentscheidungen neuer App-Läufe im gebundenen Forschungsordner unter `review/`, unveränderliche Prüfversionen unter `review/versions/`. Vorbereitete Folgeeingaben werden zusätzlich unter `review/followups/<Revision-ID>/` als `segments.csv`, `codebook.csv` und `review_snapshot.json` mit SHA-256-Inhaltsnachweis gespeichert. Die unveränderlichen internen Eingabekopien unter `revisions/` bleiben erhalten. Erst nach erfolgreicher Prüfung und externem Export wird die neue Eingabeversion im Projekt ausgewählt. Der aktuelle Ergebniszielpfad gilt für einen neuen Folgelauf; die alte Prüfung bleibt am ursprünglichen Lauf gebunden. Alte Jobs behalten `jobs/<Job-ID>/review/`. Forschungsordner und technische Projektdaten gemeinsam sichern; bei echten Forschungsdaten gehören beide nicht in ein öffentliches Repository.
